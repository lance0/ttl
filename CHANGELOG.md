...
### Security
- **`maxminddb` 0.30 → 0.31** fixes a denial of service in MMDB decoding: a crafted database could make record or metadata decoding reference shared data repeatedly and burn unbounded CPU and memory. ttl opens whichever GeoLite2 file it is pointed at, so the reader now caps the work done per lookup and returns an error instead. Also bounds subdivision lists from untrusted data.
...
### Dependencies
- dirs 6 → 7 (direct; the only change is Windows `preference_dir`, which ttl does not use), reqwest 0.13.4 → 0.13.5, hickory-resolver/-proto/-net 0.26.1 → 0.26.3, aws-lc-rs 1.16.3 → 1.18.1, aws-lc-sys 0.40.0 → 0.45.0, toml 1.1.4 → 1.1.6.
...
- **Traces no longer collapse to a single hop on macOS and other fast hosts** (#12). `sendto` is asynchronous on macOS/BSD: the kernel stamps each queued datagram with the socket's *current* IP TTL at drain time, so a rapid `setsockopt(IP_TTL); send` loop could emit every probe with the final TTL, leaving only the destination/gateway visible — intermittently, and more often on fast Apple Silicon (reproduced on the wire on an M2, macOS 26.5.1). The previous fixed 500µs delay only masked the race.
  - **IPv4 (all platforms):** every probe is now sent through a single raw `IP_HDRINCL` socket with the TTL written directly into a hand-built IP header (ICMP/UDP/TCP), removing the race by construction — no per-probe socket churn, no timing delay. The per-OS `ip_len`/`ip_off` byte order is handled (host order on macOS/NetBSD, network order on Linux/FreeBSD ≥11) and validated on real Linux, macOS, and FreeBSD kernels in CI.
